    ...
    return createPortal(
        <div ref={mainContainer} className={`fixed inset-0 bg-[#0006] ${show? 'fade':'fadeOff'}`} onClick={closingModal}>
            <div className="relative w-full h-full">
                <section ref={modal} className={`flex gap-y-3 flex-col w-[85%] md:w-[70%] lg:w-1/2 absolute top-1/2 left-1/2 translate-[-50%] bg-base-100 p-5 rounded ${show? 'showUp': 'showDown'}`} onClick={(e) => e.stopPropagation()}>
                    <div>
